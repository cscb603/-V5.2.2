...
class ImageProcessor:
    def __init__(self, input_dir, output_dir, max_side=DEFAULT_MAX_SIDE, jpg_quality=DEFAULT_JPG_QUALITY,
                 process_raw=True, high_quality_raw=False, log_queue=None, progress_queue=None):
        ...
        self.jpg_threads = max(4, self.cpu_count * JPG_THREAD_MULTIPLIER)
        self.running = True
    ...
    def run(self):
        ...
        # 优先处理所有JPG图片（保持高速处理逻辑）
        if image_files and self.running:
            self._log(f"\n开始高速处理普通图片（初始{self.jpg_threads}线程，动态调整）...")
            task_queue = queue.Queue()
            for f in image_files:
                task_queue.put(f)

            executor = concurrent.futures.ThreadPoolExecutor(max_workers=self.jpg_threads)

            # 初始提交任务
            initial_tasks = min(self.jpg_threads, task_queue.qsize())
            ...
            adjuster_thread = threading.Thread(
                target=self._jpg_worker_adjuster,
...
class ImageProcessorGUI:
    ...
    def _create_layout(self):
        ...
        # 目录选择
        ttk.Label(main_frame, text="图片文件夹:", font=('Helvetica', 10)).grid(row=2, column=0, pady=(0, 5), sticky=tk.W)
        ...
        ttk.Label(main_frame, text="保存到文件夹:", font=('Helvetica', 10)).grid(row=3, column=0, pady=(0, 5), sticky=tk.W)
        ...
        # 缩图参数（摄影优化提示）
        ttk.Label(main_frame, text="压缩参数设置 (摄影推荐):", font=('Helvetica', 10)).grid(row=4, column=0, pady=(10, 5), sticky=tk.W)

        ttk.Label(main_frame, text="图片最大边长 (像素):", font=('Helvetica', 9)).grid(row=5, column=0, pady=(0, 5), sticky=tk.W)
        ...
        ttk.Label(max_side_frame, text="建议2000-3000（平衡画质与体积）", font=('Helvetica', 8)).pack(side=tk.LEFT, padx=5)

        ttk.Label(main_frame, text="JPG质量 (1-100):", font=('Helvetica', 9)).grid(row=6, column=0, pady=(0, 5), sticky=tk.W)
        ...
        ttk.Label(quality_frame, text="建议90-95（摄影级高质量）", font=('Helvetica', 8)).pack(side=tk.LEFT, padx=5)
        ...
        self.progress_label = ttk.Label(main_frame, text="就绪", font=('Helvetica', 9))
        ...
        # 日志区域
        ttk.Label(main_frame, text="处理日志:", font=('Helvetica', 10)).grid(row=12, column=0, columnspan=2, pady=(10, 5), sticky=tk.W)
